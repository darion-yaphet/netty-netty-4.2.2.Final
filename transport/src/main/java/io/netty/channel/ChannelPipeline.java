 ...
 * 
 * 一个ChannelPipeline是一个ChannelHandler的列表，用于处理和拦截Channel的入站事件和出站操作
 * 
 * <h3>Creation of a pipeline</h3>
 *
 * Each channel has its own pipeline and it is created automatically when a new channel is created.
 *
 * 每个Channel都有一个自己的ChannelPipeline，当一个新的Channel被创建时，ChannelPipeline会自动被创建
 * 
 ...
 *
 * 以下图描述了I/O事件是如何在ChannelPipeline中处理的
 * 
 * 一个I/O事件被一个ChannelInboundHandler或ChannelOutboundHandler处理，并通过ChannelHandlerContext中的事件传播方法传递给最近的Handler
 * 
 ...
 * 
 * 一个入站事件被入站处理器从下往上处理，如图的左边所示。一个入站处理器通常处理I/O线程在图的底部生成的入站数据。
 * 入站数据通常通过实际的输入操作（如{@link SocketChannel#read(ByteBuffer)}）从远程对等方读取。
 * 如果一个入站事件超越了最顶层的入站处理器，它会被静默地丢弃，或者如果需要你的注意，会被记录下来。
 * 
 * <p>
 ...
 * 
 * 一个出站事件被出站处理器从上往下处理，如图的右边所示。一个出站处理器通常处理出站流量，如写请求。
 * 如果一个出站事件超越了最底层的出站处理器，它会被一个与{@link Channel}关联的I/O线程处理。
 ...
 * 
 * 在上面的例子中，以{@code Inbound}开头的类表示它是一个入站处理器。
 * 以{@code Outbound}开头的类表示它是一个出站处理器。
 * 
 * <p>
 ...
 * 
 * 在给定的例子配置中，当一个事件进入时，处理器的评估顺序是1, 2, 3, 4, 5。
 * 当一个事件出去时，顺序是5, 4, 3, 2, 1。
 * 基于这个原则，{@link ChannelPipeline}跳过某些处理器的评估以缩短堆栈深度：
 * 
 * <ul>
