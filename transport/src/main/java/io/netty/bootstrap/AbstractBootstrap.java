 ...
 * 
 * {@link AbstractBootstrap}是一个帮助类，它使{@link Channel}的引导变得容易。
 * 它支持方法链，提供了一种简单的方式来配置{@link AbstractBootstrap}。
 * 
 * <p>When not used in a {@link ServerBootstrap} context, the {@link #bind()} methods are useful for connectionless
 * transports such as datagram (UDP).</p>
 * 
 * 当不用于{@link ServerBootstrap}上下文时，{@link #bind()}方法对于无连接的传输（如数据报（UDP））很有用。
 * 
 */
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
     ...
     * {@link Channel}
     * 
     * 用于处理将要创建的{@link Channel}的所有事件的{@link EventLoopGroup}。
     */
     ...
     * 
     * 你可以使用这个方法或{@link #channelFactory(io.netty.channel.ChannelFactory)}，
     * 如果你的{@link Channel}实现没有无参数构造函数。
     * 
     */
     ...
     * 
     * 允许指定一个{@link ChannelOption}，用于创建后的{@link Channel}实例。
     ...
     * 
     * 允许指定一个初始属性，用于新创建的{@link Channel}。
     ...
     * 
     * 使用给定的类加载器加载{@link ChannelInitializerExtension}。
     * 
     * 默认情况下，扩展将由加载这个引导类的同一个类加载器加载。
     * 
     */
